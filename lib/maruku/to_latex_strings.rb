...
#		puts "before = #{s.inspect}"
		# We replace these by their \char sequences
		s2 = ""
		s.each_byte do |b|
...
#		puts "after = #{s2.inspect}"
	...
	def String.convert_entities(s)
		...
		r = s.gsub(/&([\w\d]+);/) do |match|
			replace = @@entity_to_latex[match]
			if replace
#				puts "#{match} -> #{replace}"
				replace
			else
				$stderr.puts "Cannot translate #{match}"
				match
			end
		end
	end
	...
	def String.init_tables
		...
		if not $conversion_table_inited
			...
			doc.elements.each("//char") do |c| 
				...
				convert.gsub!(/^\$/, "\\begin{math}")
				convert.gsub!(/\$$/, "\\end{math}")
